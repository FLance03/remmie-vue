<template>
  <info-table v-on:changePage="changePage($event)" v-bind:pageNumbers="pageNumbers" v-bind:page="currentPage" v-bind:heads="tableHead" v-bind:infos="tableData" v-bind:service="service"></info-table>
</template>
...
export default {
  data() {
    return {
      currentPage: 1,
      infoPerPage: 5,
      tableHead: ['User Name', 'Orders', 'Room', 'Status', 'Actions'],
      tableData: [],
      wholeData: [],
      pageNumbers: [],
      service: true,
  ...
  methods: {
    changePage: function(page) {
      if (page>=1 && page<=Math.ceil(this.wholeData.length/this.infoPerPage)){
    ...
    updateInfo: function() {
      var i,count;
      this.pageNumbers = pagination(this.currentPage,Math.ceil(this.wholeData.length/this.infoPerPage),2);
      for (count=0,i=(this.currentPage-1)*this.infoPerPage ; i<this.wholeData.length && count<this.infoPerPage ; i++,count++){
        this.tableData.push([]);
